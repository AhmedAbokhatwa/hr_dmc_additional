...
class CustomPayroll(PayrollEntry):
    ...
    def should_add_component_to_accrual_jv(self, component_type: str, item: dict) -> bool:
        add_component_to_accrual_jv = True
        custom_create_gl_entry, is_flexible_benefit, only_tax_impact = frappe.get_cached_value(
            "Salary Component", item["salary_component"], ["custom_create_gl_entry", "is_flexible_benefit", "only_tax_impact"]
        )
        if cint(custom_create_gl_entry):
            add_component_to_accrual_jv = True
        if not cint(custom_create_gl_entry):
            add_component_to_accrual_jv = False       
        if component_type == "earnings":
